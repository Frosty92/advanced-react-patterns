...
Just like with Context Module pattern, Compound Component pattern provides the user with a very declarative API where the inner workings are hidden, and the user is confined to literally only 
using the API as intended.

It does so by having the parent pass implicit state to children via `React.cloneElement`. 
By doing this, the user of the API doesnt have to worry about what props/state to pass to children or 
has to worry about managing state.
