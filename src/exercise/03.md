...
Flexible compound components have the benefit of having the state implicitly
passed to any descendents, whereas non-flexible ones (which use a combination of
React.Children and React.cloneElement) dont. Which one to use depends on the
use-cases for the API. If you want the state to only be shared with direct
children, use React.Childen, otherwise, use flexible compound components
